...
import org.live.streaming.provider.kafka.KafkaCodeConstants;
import org.live.streaming.provider.kafka.KafkaObject;
import org.live.streaming.provider.mapper.IUserMapper;
...
public class UserServiceImpl extends ServiceImpl<IUserMapper, UserPO> implements IUserService {
    ...
    public Map<Long, UserDTO> batchQueryUserInfo(List<Long> userIdList) {
        ...
        List<UserDTO> userDTOList = redisTemplate.opsForValue().multiGet(multiKeyList)
        ...
        // 为了防止笛卡尔全表路由 这里会对需要查询的id 进行取模分组
        ...
        // 如果 mysql 不为空  则把查询到的结果放到redis缓存中
        if (!CollectionUtils.isEmpty(dbQueryList)) {
            Map<String, UserDTO> multiSaveMap = dbQueryList.stream()
